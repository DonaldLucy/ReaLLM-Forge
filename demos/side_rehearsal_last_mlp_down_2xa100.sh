...
OUT_DIR="${OUT_DIR:-results/side_rehearsal_last_mlp_down_1xa100_20000iter}"
RUN_NAME="${RUN_NAME:-side_rehearsal_last_mlp_down_1xa100_40g}"
...
SAVE_MAJOR_CKPT_INTERVAL="${SAVE_MAJOR_CKPT_INTERVAL:-1000}"
...
LAUNCH_CMD=("${PYTHON_BIN}" "${TRAIN_ARGS[@]}")

printf '[launch]' | tee -a "${LOG_FILE}"
printf ' %q' "${LAUNCH_CMD[@]}" | tee -a "${LOG_FILE}"
printf '\n' | tee -a "${LOG_FILE}"
